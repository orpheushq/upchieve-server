...
module.exports = {
  create: function (options, cb) {
    ...
    if (!userId) {
      return cb(errors.generateError(errors.ERR_INVALID_DATA, 'Cannot create a session without a user id'), null)
    } else if (user.isVolunteer) {
      return cb(errors.generateError(errors.ERR_NOT_AUTHORIZED, 'Volunteers cannot create new sessions'), null)
    } else if (!type) {
      return cb(errors.generateError(errors.ERR_INVALID_DATA, 'Must provide a type for a new session'), null)
    }
